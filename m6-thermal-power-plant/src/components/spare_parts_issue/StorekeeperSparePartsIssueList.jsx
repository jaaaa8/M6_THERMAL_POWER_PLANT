    ...
    const handleCompleteIssue = async () => {
        ...
        if (selectedIssue.details && selectedIssue.details.length > 0) {
            for (let i = 0; i < selectedIssue.details.length; i++) {
                ...
                const actQty = parseInt(d.actualQuantity, 10);
                if (isNaN(actQty) || actQty < 0) {
                    toast.error(`Số lượng thực tế cấp của vật tư "${d.sparePartName}" không hợp lệ (phải >= 0).`);
                    return;
                }
                const stock = d.currentStock || 0;
                if (actQty > stock) {
                    toast.error(`Số lượng thực tế cấp của vật tư "${d.sparePartName}" (${actQty}) vượt quá tồn kho khả dụng (${stock})!`);
                    return;
                }
            }
        }
        ...
        try {
            const updatePayload = {
                ...
                details: selectedIssue.details ? selectedIssue.details.map(d => ({
                    ...
                    quantity: d.quantity,
                    actualQuantity: d.actualQuantity !== "" && d.actualQuantity !== null ? parseInt(d.actualQuantity, 10) : 0
                })) : []
            };
